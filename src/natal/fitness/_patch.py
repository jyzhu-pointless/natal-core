...
from natal.fitness._types import FitnessPopulationView
...
def _apply_viability_allele_scaling(
    ...
    for name in names:
        gene = deps.species.gene_index.get(name.strip())
        if gene is None:
        ...
        for m_genotype in all_genotypes:
            ...
            for f_z in f_z_indices:
                for m_z in m_z_indices:
                    ...
                    deps.config.set_sexual_selection_fitness(f_z, m_z, current * factor)
