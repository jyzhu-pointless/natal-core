...
# ── Helpers: hook genotype reference extraction (for compress seeds) ────────────

# Selector value type (equivalent to hooks.entry.selector.SelectorSpec,
# but defined here to avoid a configurator → hooks layering violation).
_SelectorVal = str | int | Sequence[str | int]


def _collect_genotype_strings(genotype_ref: str | Sequence[str]) -> set[str]:
    """Extract genotype string references from a single ``HookOp.genotypes``
    value or selector entry.

    Args:
        genotype_ref: A single genotype string (``"WT|Dr"``), a sequence
            of strings, or the wildcard ``"*"`` (returned as empty — no
            seed info).

    Returns:
        Set of non-wildcard genotype strings.
    """
    result: set[str] = set()
...
def _collect_selector_genotype_strings(selectors: dict[str, _SelectorVal]) -> set[str]:
    """Extract genotype strings from ``@hook(selectors=...)`` values."""
    result: set[str] = set()
...
def _extract_hook_genotype_refs(hook_items: list[_HookItem]) -> set[str]:
    """Extract genotype string references from hook items for compression seeds.

    Iterates all hook items registered via ``.hooks()`` and extracts genotype
    pattern strings that would be introduced by hooks at runtime.

    - **Selector hooks**: reads ``func.selectors`` metadata directly.
    - **Declarative hooks**: calls the function once to obtain ``HookOp``
      objects and extracts ``op.genotypes`` fields.
    - **Custom hooks**: skipped — operate on raw integer indices.
    """
    refs: set[str] = set()
    for item in hook_items:
        if isinstance(item, dict):
            ...
            for registrations in hook_dict.values():
                for hook_reg in registrations:
                    func: Callable[..., Any] = hook_reg[0]
                    refs.update(_extract_refs_from_callable(func))
        elif callable(item):
...
def _extract_refs_from_callable(func: Callable[..., Any]) -> set[str]:
    ...
    if selectors:
        return _collect_selector_genotype_strings(selectors)

    meta = getattr(func, "meta", None)
    is_custom = getattr(func, "custom", False) or (meta and meta.get("custom", False))
    if not is_custom and meta:
        try:
            result = func()
            if isinstance(result, list):
                refs: set[str] = set()
                for op in result:  # pyright: ignore[reportUnknownVariableType]
                    genotypes: object = getattr(op, "genotypes", None)  # pyright: ignore[reportUnknownArgumentType]
                    if isinstance(genotypes, str):
                        refs.update(_collect_genotype_strings(genotypes))
                    elif isinstance(genotypes, (list, tuple)):
                        for g in genotypes:  # pyright: ignore[reportUnknownVariableType]
                            if isinstance(g, str):
                                refs.update(_collect_genotype_strings(g))
                return refs
        except Exception:
            pass
...
class Configurator:
    ...
    def build(
        ...
        if self._compress and not self._compression_applied:
            ...
            # only via hooks survive BFS pruning without requiring manual
            # declared_zygote_types.
            stored_hooks: list[_HookItem] | None = getattr(
                self, "_hook_items", None
            )
            if stored_hooks:
                hook_refs = _extract_hook_genotype_refs(stored_hooks)
                if hook_refs:
