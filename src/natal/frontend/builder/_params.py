...
Function overview:
  ...
  - ``iter_sexual_selection_entries()`` — flatten the flexible
    sexual-selection declaration into ``(female, male, weight)`` rows.

  The initial-state resolvers (``resolve_age_structured_initial_*``,
  ``resolve_discrete_initial_individual_count``) live with the model
  assembly side in :mod:`natal.frontend.model.initial_state`.
"""
