...
point between the hook system (``natal.frontend.hooks.*``) and the population
simulation loop.  It:
...
is embedded from ``natal.backends.numba.lifecycle``, so there are no engine
templates.
"""
...
from natal.frontend.hooks.compile.codegen import (
...
def compile_lifecycle_wrapper(
    ...
    ``natal.backends.numba.lifecycle`` and are assembled into a generated module by
