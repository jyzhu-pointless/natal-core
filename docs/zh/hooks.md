...
`initialization` 适合做一次性的启动逻辑，例如：

1. 设置初始边界条件或阈值。
2. 预先记录实验元数据。
3. 在首个 tick 前完成一次状态修正或检查。

选择事件时，建议先明确你的干预发生在“初始化前”、“生存前”还是“生存后”，这会显著影响结果解释。

## 3. 推荐写法：声明式 Hook

对于大多数用户，推荐使用 `@hook` 与 `Op.*`，然后用 `set_hook(...)` 绑定到 population：
...
1. 用 `@hook` 定义规则。
2. 优先用 `set_hook(...)` 绑定到 population。
3. 如需保留 `@hook` 的便捷注册写法，再使用 `register(...)`。
4. 用 `run(...)` 执行并结合 history 验证结果。
