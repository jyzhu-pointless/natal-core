...
> **注意**：重组率设置依赖于位点的顺序，位点顺序由 `position` 参数控制。详见 Locus 章节中的 [关于 `position` 参数](#position)。
...
`Haplotype` 表示单条染色体上所有位点的等位基因组合。对于包含 $N$ 个位点的染色体，可能的单倍型数量为所有位点等位基因数的乘积：$\prod_{i=1}^N \text{每个位点的等位基因数}$。
...
- v0.1.x 中 `unordered` 默认值为 `False`。v0.2.0 改为 `True`。
- 依赖 `A|a ≠ a|A` 区分的代码需要显式设置 `unordered=False`。
...
- [快速开始：15 分钟上手 NATAL](1_quickstart.md) - 基本使用示例
- [种群初始化](2_population_initialization.md) - 从 Species 到可运行种群的链式构建
- [IndexRegistry 索引机制](4_index_registry.md) - 对象索引的详细机制
- [Genotype Pattern 语法与匹配](2_genotype_patterns.md) - 基因型模式表达、`|`/`::` 顺序规则与匹配示例
- [Modifier 机制](3_modifiers.md) - 如何基于 Genotype 定义遗传规则
