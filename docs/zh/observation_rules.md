...
如果你的目标是“直接拿到可读结构用于日志、接口返回或落盘”，可以不手动调用
`ObservationFilter + apply_rule`，而是直接使用状态翻译模块里的 observation 集成接口。

这在 [PopulationState 与 PopulationConfig](population_state_config.md) 的
“状态翻译为可读字典/JSON”一节中也有对应入口。
...
# 字典输出：适合 API 返回、调试打印
obs_dict = nt.population_to_observation_dict(
    pop,
    groups=groups,
    collapse_age=False,
)
print(obs_dict["observed"]["adult_wt_female"])

# JSON 输出：适合持久化或跨进程传输
obs_json = nt.population_to_observation_json(
    pop,
    groups=groups,
    collapse_age=False,
    indent=2,
...
- 需要和外部系统交换数据时，优先 `population_to_observation_json`。
- 需要在 Python 内继续计算时，优先 `population_to_observation_dict`。
