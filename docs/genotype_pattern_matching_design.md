# 设计自己的 Preset（2）：模式匹配与可扩展配置

本章继续主线，解决一个关键基础问题：

**如何用高效、可读的方式描述和过滤基因型？**

模式匹配不仅用于 Preset 设计，也是 **Observation（观察与过滤）系统的核心特性**。掌握模式匹配，你既能在 Observation 中精确定义观次群体，也能在 Preset 中优雅地指定规则作用范围。

前面的第 1 章已经完成：

1. 规则定义（Gamete 与 Zygote 转换）。

本章解决如何精确描述"这条规则该对哪些基因型生效"的问题。
