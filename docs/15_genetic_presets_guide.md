...
- [等位基因转换规则](11_allele_conversion_rules.md) - 详细的转换规则系统
- [基因型模式匹配](14_genotype_pattern_matching_design.md) - 高级基因型过滤
- [Modifier机制](06_modifiers.md) - 底层修饰器原理
...
- `tests/test_recipe_fitness_patch.py` - 适应度补丁测试# 遗传预设使用指南 (Genetic Presets Guide)

本文档介绍如何使用和创建遗传预设（Genetic Presets），包括基因驱动、突变系统和其他遗传修饰。

## 概述

**遗传预设**（之前称为 *recipe*）是NATAL框架中用于定义可重用遗传修饰的机制。预设可以：
...
pop = (AgeStructuredPopulationBuilder(species)
       .setup(name="TestPop")
       .age_structure(n_ages=8)
       .initial_state({"female": {"WT|WT": 100}})
       .presets(preset1, preset2)  # 可以应用多个预设
       .build())
