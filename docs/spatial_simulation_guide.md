...
### 5.2 最新性能结果（2026-04-03）

在清理缓存并统一预热/计时口径后，`demos/spatial_hex.py` 的同一组场景（100x100 HexGrid，`spatial.run(5)`）
端到端耗时已经降到 **0.6-0.7s** 量级（稳定区间）。

这次结果和早期 2.8-3.0s 量级看起来差异很大，核心原因是：

1. 旧测量混入了不一致的缓存状态与基线口径。
2. 新实现把 migration 的路由复杂度从“容易退化到全量扫描”改成了 kernel 稀疏邻域驱动。
3. reproduction 和 migration 阶段都减少了重复分配与重复计算。

如果你要复现这个量级，建议固定下面的流程：

1. 固定模型参数与随机模式。
2. 做一次预热运行（避免首次编译开销污染）。
3. 连续多次计时并报告区间，而不是只看一次最快值。
