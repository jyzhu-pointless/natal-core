...
**A**:
- `stochastic=False`: uses the expectation of a multinomial distribution – results are completely deterministic (may be fractional)
- `stochastic=True`: uses random sampling – results are stochastic (always integers)
