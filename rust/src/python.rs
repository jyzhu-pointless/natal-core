...
//!
//! These are the module-level functions the extension exports: the two
//! aging kernels, the CSR migration entry points, and the equilibrium
//! metric helpers.  Session objects live in `sessions`.
...
#[pyfunction]
pub(crate) fn age_structured_aging(
...
#[pyfunction]
pub(crate) fn discrete_aging(mut individual_count: PyReadwriteArray3<'_, f64>) -> PyResult<()> {
    let ind_shape = individual_count.shape();
    let (n_sexes, n_ages, n_ztypes) = (ind_shape[0], ind_shape[1], ind_shape[2]);
