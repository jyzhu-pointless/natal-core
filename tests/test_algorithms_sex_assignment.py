import numpy as np

from natal.backends.reference.simulation.age_structured import fertilize_with_precomputed_offspring_probability_and_age_specific_reproduction


def test_fertilize_offspring_sex_is_genotype_determined_when_constrained() -> None:
    n_ztypes = 2
    female_counts = np.zeros((1, n_ztypes), dtype=np.float64)
    sperm_store = np.zeros((1, n_ztypes, n_ztypes), dtype=np.float64)
    sperm_store[0, 0, 0] = 10.0

    fertility_f = np.ones(n_ztypes, dtype=np.float64)
    fertility_m = np.ones(n_ztypes, dtype=np.float64)

    offspring_probability = np.zeros((n_ztypes, n_ztypes, n_ztypes), dtype=np.float64)
    offspring_probability[0, 0, 0] = 0.5
    offspring_probability[0, 0, 1] = 0.5

    female_compat = np.array([1.0, 0.0], dtype=np.float64)
    male_compat = np.array([0.0, 1.0], dtype=np.float64)
    female_only = np.array([True, False], dtype=np.bool_)
    male_only = np.array([False, True], dtype=np.bool_)

    n_female, n_male = fertilize_with_precomputed_offspring_probability_and_age_specific_reproduction(
        female_counts=female_counts,
        sperm_storage_by_male_genotype=sperm_store,
        fertility_f=fertility_f,
        fertility_m=fertility_m,
        offspring_probability=offspring_probability,
        average_eggs_per_wt_female=1.0,
        adult_start_idx=0,
        n_ages=1,
        n_ztypes=n_ztypes,
        n_haplogenotypes=1,
        female_ztype_compatibility=female_compat,
        male_ztype_compatibility=male_compat,
        female_only_by_sex_chrom=female_only,
        male_only_by_sex_chrom=male_only,
        n_glabs=1,
        age_based_reproduction_rates=np.ones(1, dtype=np.float64),
        female_age_based_fertility=np.ones(1, dtype=np.float64),
        fixed_eggs=True,
        sex_ratio=0.9,
        has_sex_chromosomes=True,
        stochastic=False,
        continuous_sampling=False,
    )

    assert np.allclose(n_female, np.array([5.0, 0.0]))
    assert np.allclose(n_male, np.array([0.0, 5.0]))


def test_fertilize_offspring_sex_uses_sex_ratio_without_constraints() -> None:
    n_ztypes = 2
    female_counts = np.zeros((1, n_ztypes), dtype=np.float64)
    sperm_store = np.zeros((1, n_ztypes, n_ztypes), dtype=np.float64)
    sperm_store[0, 0, 0] = 10.0

    fertility_f = np.ones(n_ztypes, dtype=np.float64)
    fertility_m = np.ones(n_ztypes, dtype=np.float64)

    offspring_probability = np.zeros((n_ztypes, n_ztypes, n_ztypes), dtype=np.float64)
    offspring_probability[0, 0, 0] = 1.0

    compat = np.ones(n_ztypes, dtype=np.float64)
    none_only = np.zeros(n_ztypes, dtype=np.bool_)

    n_female, n_male = fertilize_with_precomputed_offspring_probability_and_age_specific_reproduction(
        female_counts=female_counts,
        sperm_storage_by_male_genotype=sperm_store,
        fertility_f=fertility_f,
        fertility_m=fertility_m,
        offspring_probability=offspring_probability,
        average_eggs_per_wt_female=1.0,
        adult_start_idx=0,
        n_ages=1,
        n_ztypes=n_ztypes,
        n_haplogenotypes=1,
        female_ztype_compatibility=compat,
        male_ztype_compatibility=compat,
        female_only_by_sex_chrom=none_only,
        male_only_by_sex_chrom=none_only,
        n_glabs=1,
        age_based_reproduction_rates=np.ones(1, dtype=np.float64),
        female_age_based_fertility=np.ones(1, dtype=np.float64),
        fixed_eggs=True,
        sex_ratio=0.3,
        has_sex_chromosomes=False,
        stochastic=False,
        continuous_sampling=False,
    )

    assert np.allclose(n_female, np.array([3.0, 0.0]))
    assert np.allclose(n_male, np.array([7.0, 0.0]))


def test_fertilize_offspring_sex_ignores_asymmetric_compat_when_unconstrained() -> None:
    n_ztypes = 2
    female_counts = np.zeros((1, n_ztypes), dtype=np.float64)
    sperm_store = np.zeros((1, n_ztypes, n_ztypes), dtype=np.float64)
    sperm_store[0, 0, 0] = 10.0

    fertility_f = np.ones(n_ztypes, dtype=np.float64)
    fertility_m = np.ones(n_ztypes, dtype=np.float64)

    offspring_probability = np.zeros((n_ztypes, n_ztypes, n_ztypes), dtype=np.float64)
    offspring_probability[0, 0, 0] = 0.5
    offspring_probability[0, 0, 1] = 0.5

    female_compat = np.array([1.0, 0.0], dtype=np.float64)
    male_compat = np.array([0.0, 1.0], dtype=np.float64)
    none_only = np.zeros(n_ztypes, dtype=np.bool_)

    n_female, n_male = fertilize_with_precomputed_offspring_probability_and_age_specific_reproduction(
        female_counts=female_counts,
        sperm_storage_by_male_genotype=sperm_store,
        fertility_f=fertility_f,
        fertility_m=fertility_m,
        offspring_probability=offspring_probability,
        average_eggs_per_wt_female=1.0,
        adult_start_idx=0,
        n_ages=1,
        n_ztypes=n_ztypes,
        n_haplogenotypes=1,
        female_ztype_compatibility=female_compat,
        male_ztype_compatibility=male_compat,
        female_only_by_sex_chrom=none_only,
        male_only_by_sex_chrom=none_only,
        n_glabs=1,
        age_based_reproduction_rates=np.ones(1, dtype=np.float64),
        female_age_based_fertility=np.ones(1, dtype=np.float64),
        fixed_eggs=True,
        sex_ratio=0.3,
        has_sex_chromosomes=False,
        stochastic=False,
        continuous_sampling=False,
    )

    assert np.allclose(n_female, np.array([1.5, 1.5]))
    assert np.allclose(n_male, np.array([3.5, 3.5]))
