...
from natal.frontend.data import DiscretePopulationState, PopulationState, parse_flattened_discrete_state, parse_flattened_state
...
class TestPopulationStateCreate:
    def test_default_creation(self):
        state = PopulationState.create(n_ztypes=3)
    ...
    def test_custom_dimensions(self):
        state = PopulationState.create(n_ztypes=5, n_sexes=3, n_ages=4, n_tick=10)
    ...
    def test_provided_individual_count(self):
        ...
        state = PopulationState.create(n_ztypes=3, n_sexes=1, n_ages=2, individual_count=arr)
    ...
    def test_provided_sperm_storage(self):
        ...
        state = PopulationState.create(n_ztypes=2, n_ages=1, sperm_storage=sperm)
    ...
    def test_n_ztypes_zero_raises(self):
        with pytest.raises(AssertionError, match="n_ztypes must be positive"):
            PopulationState.create(n_ztypes=0)

    def test_n_ages_zero_raises(self):
        with pytest.raises(AssertionError, match="n_ages must be positive"):
            PopulationState.create(n_ztypes=2, n_ages=0)

    def test_individual_count_shape_mismatch_raises(self):
        arr = np.ones((2, 2, 3))  # expected (2, 2, 5) for n_ztypes=5
        with pytest.raises(AssertionError, match="Invalid shape for individual_count"):
            PopulationState.create(n_ztypes=5, individual_count=arr)

    def test_sperm_storage_shape_mismatch_raises(self):
        sperm = np.ones((2, 3, 3))  # expected (2, 5, 5) for n_ztypes=5
        with pytest.raises(AssertionError, match="Invalid shape for sperm_storage"):
            PopulationState.create(n_ztypes=5, n_ages=2, sperm_storage=sperm)
...
class TestPopulationStateAccessors:
    @pytest.fixture
    def state(self):
        return PopulationState.create(n_ztypes=3, n_sexes=2, n_ages=2)
...
class TestDiscretePopulationState:
    def test_create_default(self):
        state = DiscretePopulationState.create(n_sexes=2, n_ages=2, n_ztypes=4)
        assert state.n_tick == 0
    ...
    def test_create_custom_tick(self):
        state = DiscretePopulationState.create(n_sexes=2, n_ages=2, n_ztypes=4, n_tick=42)
        assert state.n_tick == 42

    def test_create_provided_array(self):
        arr = np.array([[[1.5, 2.5]]])
        state = DiscretePopulationState.create(
            n_sexes=1, n_ages=1, n_ztypes=2, individual_count=arr
        )
    ...
    def test_flatten_all_roundtrip(self):
        ...
        state = DiscretePopulationState.create(
            n_sexes=n_sexes, n_ages=n_ages, n_ztypes=n_genotypes, n_tick=7
        )
        state.individual_count[1, 2, 3] = 99.0
        flat = state.flatten_all()
        restored = parse_flattened_discrete_state(
            flat, n_sexes=n_sexes, n_ages=n_ages, n_ztypes=n_genotypes
        )
...
class TestPopulationStateFlatten:
    def test_flatten_all_roundtrip(self):
        ...
        state = PopulationState.create(
            n_ztypes=n_genotypes, n_sexes=n_sexes, n_ages=n_ages, n_tick=5
        )
